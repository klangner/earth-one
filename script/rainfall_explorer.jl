...
@manipulate for station in stations[stations.rain, :no],
    ...
    ymax = length(df2.rain) > 0 ? 1.2*maximum(df2.rain) : 10
    scatter(df2.time, df2.rain, title="Station $station",
            marker=([:dot], 2, 2, Plots.stroke(0))
end
